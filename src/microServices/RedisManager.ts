...
export default class RedisManger {
     ...
     */
    public async set(key: string, value: number | string): Promise<string> {
    ...
    public pipeline(): Pipeline {
        return this.redis.pipeline() as any;
    }
    /**
     * 讓一個key有過期時間 單位是秒
     * @param {string} key
     ...
     * @memberof RedisManger
     */
    public async hmgetArray(key: string, fields: string[]) {
     ...
     * @memberof RedisManger
     */
    public async hmsetArray(key: string, fields: any[]): Promise<0 | 1> {
     ...
     * @memberof RedisManger
     */
    public async hmsetObject(key: string, obj: any): Promise<0 | 1> {
     ...
     * @memberof RedisManger
     */
    public async execPipeline(callback: (err: Error, res: any) => void) {
     ...
     * @memberof RedisManger
     */
    public async geoadd(key: string, geoEntity: GEOEntity[]) {
     ...
     * @memberof RedisManger
     */
    public async geopos(key: string, member: string, ...members: string[]) {
        const redis = this.redis as any;
     ...
     * @memberof RedisManger
     */
    public async geodist(key: string, member1: string, member2: string, unit: 'm' | 'km' | 'mi' | 'ft' = 'm') {
        const redis = this.redis as any;
    ...
    public async georadius(
        key: string, longitude: number, latitude: number, radius: number,
        unit: 'm' | 'km' | 'mi' | 'ft' = 'm', ...args: any[]) {
        const redis = this.redis as any;
     ...
     * @memberof RedisManger
     */
    public async geohash(key: string, member: string, ...members: string[]) {
        const redis = this.redis as any;
